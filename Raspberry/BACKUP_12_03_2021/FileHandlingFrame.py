...
from MainApplication import MainApplication as main
...
class FileHandlingFrame(Tk.LabelFrame):
    ...
    def create_widgets(self):
        ...
        self.networkDropdown = ttk.OptionMenu(self, self.networkVarList)
    ...
    def createHeatmapButtonClicked(self):
        ...
        if (self.master.workingDataSeries is None):
            mb.showerror(master=self.master, title="No data series available", message="Pipi")
            return 
        if not (type(self.master.workingDataSeries) == DataSeries):
            mb.showerror(master=self.master, title="No data series available", message="Pupu")
            return

        self.heatmapTest(self.master.workingDataSeries)
    ...
    def heatmapTest(self, dataseries):
        h = Heatmapper(dataseries)
        h.test()
