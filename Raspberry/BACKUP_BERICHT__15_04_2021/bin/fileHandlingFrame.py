...
class FileHandlingFrame(Tk.LabelFrame):
    ...
    def createHeatmapButtonClicked(self):
        ...
        # --< Create heatmap >--------------------------------------------
        hm = Heatmapper(self, self.master.workingDataSeries, hm_config)
        hm.create()

        return

    def heatmapSettingsButtonClicked(self):
        mb.showinfo(master=self.master, title="No funcionality implemented yet.", message="This functionality is not implemented yet. ¯\_(ツ)_/¯")
        return
    ...
    def importData(self):
        ...
        if(self.master.workingFile == ""):
            # mb.showinfo(master=self.master, title="No file selected", message="No file selected.") # not needed, but if you want you can enable it 
            return
