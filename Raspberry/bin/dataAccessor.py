...
class DataAccessor:
    ...
    def addDataPoint(self, x, y, dataSeries):
        ...
        for j in range(numberOfNetworksFound):
            ...
            # --< get rssi values >---------------------------------------
            rssi_tmp = self.serialPort.read(DataAccessor.RSSI_NUMBER_OF_BYTES)
            rssiCheck_tmp = self.serialPort.read(DataAccessor.RSSI_NUMBER_OF_BYTES)

            # Check whether reading from data port was successful             
            if (type(rssi_tmp) == str) or (type(rssiCheck_tmp) == str):
                # Mucho problem
                self.serialPort.reset_input_buffer()
                raise RssiValueTransmissionException("Rssi timeout")

            # Convert to useful data
            rssi = int.from_bytes(rssi_tmp, byteorder = 'big', signed = True)
            rssiCheck = int.from_bytes(rssiCheck_tmp, byteorder = 'big', signed = True)
            # 
            ...
            ssid = ssid.rstrip('\x00') # remove all '\x00'
